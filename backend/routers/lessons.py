...
def normalize_lesson(data: dict) -> dict:
    ...
    # Mirror the grammar_focus→grammar_discovery mapping inside the baked
    # "somali" block so the UI finds translations at the normalized path.
    som = data.get("somali")
    if isinstance(som, dict) and "grammar_focus" in som and "grammar_discovery" not in som:
        som["grammar_discovery"] = {
            "sections": [
                {"explanation": g.get("explanation", "")}
                for g in som["grammar_focus"]
